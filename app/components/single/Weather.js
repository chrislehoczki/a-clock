...
var maxTemp = d3.max(data, function(d) { return d.avgTemp})
var minTemp = d3.min(data, function(d) { return d.avgTemp})
var maxPrec = d3.max(data, function(d) { return d.totalPrec})
var minPrec = d3.min(data, function(d) { return d.totalPrec})
...
var tooltip = d3.select(containerId).append("div").attr("class", "graph-tooltip").style("opacity", 0)
  ...
  svg.append("rect")
      ...
      .attr("width", width)
      .attr("height", height)
      .on("mouseover", function() { tooltip.style("opacity", 0.6); })
...
var lineFunction = d3.svg.line()
                          .x(function(d, i) { return xAx(d3_time_months[i])})
                          .y(function(d) { 
...
if (checkData(data, value)) {
     ...
     //CREATE CIRCLES
    var scatterPlotGroups = svg.selectAll("." + groupName)
        .data(data)
        .enter().append("g")
    ...
    var circles = scatterPlotGroups.selectAll("circle")
        .data(data)
        .enter().append("circle")
        .attr("cx", function(d, i) { return xAx(d3_time_months[i])})
