...
function Weather (lat, long, token) {
	...
	this.monthlyAverages = function () {
		...
		var self = this;
		this.getStationId(this.lat, this.long).then(function(data) {
	...
	this.getYears = function() {
		...
		for (var i = 0; i < 3; i++) {
			getYears.push(this.getStationWeather(id, "GHCNDMS", ["DP05", "DT90", "MNTM", "TPCP"], startDates[i], endDates[i], 50))
		}
		
		Q.all(getYears).then(function(data) {
	...
	this.getStationWeather = function(stationId, dataset, dataTypes, startDate, endDate, limit) {
		var self = this;
		var deferred = Q.defer();
	...
	this.getStationId = function (lat, long) {
		...
		var url = "http://www.ncdc.noaa.gov/cdo-web/api/v2/stations/?sortfield=mindate&datasetid=GHCNDMS&extent=" + createBoundary(this.lat,this.long);
