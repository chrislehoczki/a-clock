...
module.exports = function (originName, destinationName, countryCode, currencyName, localeCode, dateYYYYMMDD) {
...
}).then(function(data) { //GET SESSION

	return getSession();
}).then(function(url) { //POLL FLIGHT DATA
	return pollData(url)
}).then(function(priceObj) { //LOG OUT PRICE OBJ WITH PRICE AND URL
...
var url = "http://partners.api.skyscanner.net/apiservices/autosuggest/v1.0/" + country + "/" + currency + "/" + locale + "?query=" 
			+ query + "&apiKey=" + key;
